		...
		(return
			(cond
				...
				((consp expr) (multiple-value-setq (sym fl) (get-variable (car expr) *envts*)) ;;; expr is an s-expression
											(cond
												((eql fl t)(lambda-run (car expr) sym (mapcar #'(lambda (x) (evall x env)) (cdr expr)))) ;; f must be a function call
												((consp (car expr)) (evall `(set! __INTERNTEMP ,(car expr))) ;; ((lambda (x) (* x x)) 2) -> 2
																						(setq ans (evall `(__INTERNTEMP ,@(mapcar #'(lambda (x) (evall x env)) (cdr expr)))))
																						(remhash '__INTERNTEMP *sym-table*)
																						ans)
												((equal 'if (car expr)) (setf expr (if (evall (second expr) env)
																														 (third expr) 
																														 (fourth expr)))
																								(go :EVALL))
												...
												((equal 'begin (car expr)) (pop expr)
																									 (loop while (rest expr) do (evall (pop expr) env))
												...
												(t ;; must be a built in function 
													...
													(if (eql flag t)
														(apply op (mapcar #'(lambda (x) (evall x env)) (rest expr)))))))))))
